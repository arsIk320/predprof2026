...
def get_subscription_day_price(cursor, meal_type: str) -> float:
    ...
    # Фолбек: если настройки еще не заполнены
    if val is None:
        try:
...
def seed_default_menu_schedule(cursor, start_date, end_date, per_day: int = 3):
    ...
    for d in _iter_dates(start_date, end_date):
        ...
        # Детерминированно "вращаем" список блюд, чтобы не копить новые записи при каждом запуске
...
def init_db():
    ...
    # Доп. поля для выбора карты при оплате (нужно для UX; хранится только идентификатор и последние 4 цифры)
    try:
    ...
    # Дополнительные позиции в меню (для большего выбора)
    cursor.execute(
        "INSERT OR IGNORE INTO menu_items (name, category, price, description, allergens) VALUES (?, ?, ?, ?, ?)",
    ...
    # Дефолтные тарифы на абонементы (₽/день). Если админ еще не задавал — используем цены из меню.
    try:
        ...
        # Заполним календарь меню демо-данными, чтобы неделя/месяц не были пустыми и был выбор блюд
    # (INSERT OR IGNORE делает операцию безопасной при повторных запусках).
    try:
...
def mark_notification_read(notification_id):
    ...
    # Проверяем доступ к уведомлению (чтобы нельзя было отмечать чужие)
    row = cursor.execute(
        f"""
        SELECT id
        FROM notifications
...
def get_menu_calendar():
    ...
    # если календарь пустой (например, в новой БД), заполним демо-меню
    try:
...
def make_payment():
    ...
    # Карта, выбранная пользователем при оплате (если карт несколько).
    # Важно: данные карты в открытом виде не храним — только id (клиентский uuid) и последние 4 цифры.
    ...
    # На всякий случай: если база старая, добавим колонки.
    try:
        ensure_column(cursor, 'payments', 'card_id', 'TEXT')
        ensure_column(cursor, 'payments', 'card_last4', 'TEXT')
    except Exception:
    ...
    # 1) Разовая оплата: сумма вводится пользователем
    ...
    # 2) Абонемент: сумма считается автоматически (стоимость за день * кол-во дней)
    # Важно: абонемент покупается за счёт баланса пользователя.
    # Ранее абонемент создавался без списания средств, из-за чего можно было
    # «оформить» абонемент бесплатно. Ниже исправление: проверяем баланс и
    ...
    if payment_type == 'subscription':
        ...
        # Списываем стоимость абонемента с баланса ученика
    ...
    # Возвращаем актуальный баланс, чтобы интерфейс мог обновиться сразу без перезагрузки
    new_balance = None
    try:
...
def process_meal_claim(user_id, meal_type, issuer_id, menu_item_id=None):
    ...
    try:
        ...
        if existing:
            return False, 'Вы уже получили это питание сегодня'
        ...
        for product_name, need in MEAL_CONSUMPTION.get(meal_type, {}).items():
            row = cursor.execute(
                "SELECT id, quantity, unit FROM products WHERE name = ? ORDER BY id ASC LIMIT 1",
                (product_name,)
            ).fetchone()
            ...
            if qty < need:
                if unit:
                    missing.append(f"{product_name} (есть {qty} {unit}, нужно {need} {unit})")
                else:
                    missing.append(f"{product_name} (нужно {need})")

        if missing:
            return False, 'Недостаточно продуктов: ' + ', '.join(missing)
        for product_name, need in MEAL_CONSUMPTION.get(meal_type, {}).items():
            row = product_rows.get(product_name)
            if not row:
                continue
            cursor.execute(
        ...
        try:
            ...
            pass
        # student_received: NULL (ожидает подтверждения учеником), если питание выдал сотрудник.
        # Если ученик отмечает сам (issuer_id == user_id), считаем подтвержденным сразу.
        ...
        # Уведомление ученику о выдаче/отметке
        try:
...
def search_students():
    ...
    # SQLite по умолчанию плохо делает case-insensitive для кириллицы,
    # поэтому пробуем несколько вариантов регистра.
    variants = {
        query,
        query.title(),
        query.capitalize(),
...
def issue_meal():
    data = request.json or {}
    # Новый вариант: выдача по ФИО/ID ученика (без логина)
    ...
    # 1) Если пришёл ID — используем его
...
def get_cook_dishes():
    ...
    try:
        ...
        result = []
        for dish in dishes:
            # Упрощенная версия - в реальности нужна таблица dish_ingredients
            ingredients = []
...
def confirm_meal_claim(claim_id: int):
    ...
    try:
        row = cursor.execute(
            "SELECT id FROM meal_claims WHERE id = ? AND user_id = ?",
